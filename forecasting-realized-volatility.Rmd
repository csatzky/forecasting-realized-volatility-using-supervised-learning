...
For this project, [SHARELAB LIMITED](https://sharelab.com) provided data containing daily _realized volatility_ (RV) of EUR/USD currency quotes. The data consists of a `r text_spec("training", color="black", background="#EEEEEE")` set (data from 1/1/2010 to 12/31/2019), and a `r text_spec("validation", color="black", background="#EEEEEE")` set (data from 1/1/2020 to 7/31/2020). The objective of this project is to predict $\textrm{RV}_{t+1}$ (i.e. EUR/USD realized volatility for date $t + 1$) as accurately as possible on the pseudo out-of-sample `r text_spec("validation", color="black", background="#EEEEEE")` set.

Any analysis and modeling is strictly performed on the `r text_spec("training", color="black", background="#EEEEEE")` dataset, while the `r text_spec("validation", color="black", background="#EEEEEE")` subset is used only to assess the final model's performance per machine-learning method considered. The performance measure is the root mean square error (RMSE).
...
Of all methods and models considered, the gradient boosted tree algorithm performs best with an RMSE of `r round(0.0015951,4)` on the `r text_spec("validation", color="black", background="#EEEEEE")` set. However, it is important to acknowledge that the `r text_spec("validation", color="black", background="#EEEEEE")` set spawns from January 2020 to July 2020, including a period of market turmoil due to the Covid-19 pandemic that greatly impacted markets in March 2020. [Rigorous analysis of the results][Predictions During Pre-Crisis Period (January 2020)] shows that;
...
Finally, using the RMSE information on the `r text_spec("test", color="black", background="#EEEEEE")` subset, _one_ model _per algorithm_ is chosen to be fit on the complete `r text_spec("training", color="black", background="#EEEEEE")` set and evaluated on the `r text_spec("validation", color="black", background="#EEEEEE")` set. Considering the RMSE performance on the `r text_spec("validation", color="black", background="#EEEEEE")` set of each method's final model, the suitability of different algorithms to forecast volatility for the EUR/USD currency is discussed.
...
Data exploration and any modeling is strictly performed on the `r text_spec("train", color="black", background="#EEEEEE")` set, which contains 90% of the observations given in the `r text_spec("training", color="black", background="#EEEEEE")` dataset.
...
First, I am describing the provided variables for the `r text_spec("train", color="black", background="#EEEEEE")` partition using summary statistics.
...
* the time-series for the `r text_spec("train", color="black", background="#EEEEEE")` partition spawns from 1 January 2010 to 31 December 2018
* the `r text_spec("date", color="black", background="#EEEEEE")` variable is correctly formatted as `r text_spec("Date", color="black", background="#EEEEEE")` and the other variables are all `r text_spec("numeric", color="black", background="#EEEEEE")`
...
First, the variable `r text_spec("date", color="black", background="#EEEEEE")` is used to extract `r text_spec("weekday", color="black", background="#EEEEEE")`.
...
Secondly, I am plotting 95% confidence intervals for the _mean value_ of $RV^d$ per `r text_spec("weekday", color="black", background="#EEEEEE")`. Note that stratifying $RV^d$ is equivalent to stratifying $RV_{t+1}$, but for the latter the `r text_spec("weekday", color="black", background="#EEEEEE")` is shifted by one day.
...
Note that _none_ of the 95% confidence intervals overlaps with the overall average value of `r round(mean(train$rv_d),5)`. This is reasonable evidence to assume some variability across weekdays and hence I will include the newly created `r text_spec("weekday", color="black", background="#EEEEEE")`-variable for modeling via the creation of dummy variables.
...
$i$: Value of `r text_spec("weekday", color="black", background="#EEEEEE")`, $i \in \{\textrm{Monday}, \textrm{Tuesday}, \textrm{Wednesday}, \textrm{Thursday}, \textrm{Friday}\}$

To avoid perfect multicollinearity, it is necessary to only create dummy variables for $N-1$ categories of `r text_spec("weekday", color="black", background="#EEEEEE")`. Hence, I am only including dummy variables for Monday to Thursday. The library `r text_spec("fastDummies", color="black", background="#EEEEEE")` (@fastdummies) provides a convenient function for dummy variable extraction.
...
The benchmark model is simply the average value of $RV_{t+1}$, without using any of the other variables. It can be directly evaluated on the `r text_spec("test", color="black", background="#EEEEEE")` set.
...
For the first linear model variant, I am including all features except for the `r text_spec("weekday", color="black", background="#EEEEEE")`  dummy variables. This model is equivalent to Corsi's (2009) Heterogeneous Autoregressive model of Realized Volatility (HAR-RV).
...
kable(summary(fit_har_rv)$coef, escape = TRUE, booktabs=TRUE) %>%
  kable_styling(position = "center")
```
As seen above, all coefficient estimates in the HAR-RV model are significantly different from zero. Also note that all coefficients are positive, which is in line with what was discovered in the [Relationship Across Variables] section. Below, I am summarizing the model's RMSE performance on the `r text_spec("test", color="black", background="#EEEEEE")` set.
...
```
The HAR-RV model greatly outperforms the benchmark model. The RMSE on the `r text_spec("test", color="black", background="#EEEEEE")` set improves by `r 100*(-round((rmse_har_rv/rmse_benchmark -1),4))`%.

### Weekday Effect Model

For the second linear model I am adding _all_ features, including dummy variables $D^i$ for the newly created `r text_spec("weekday", color="black", background="#EEEEEE")` variable. More specifically, for 5 workdays, 4 dummy variables are introduced to capture time dependencies across weekdays as shown in the [data exploration section][Stratification by Day of the Week].
...
kable(summary(fit_weekday_linear)$coef, escape = TRUE, booktabs=TRUE) %>%
  kable_styling(position = "center")
```
Each dummy variable has a parameter value that is significantly different from zero ($\textrm{pval} < 0.05$). Note that after accounting for the variability across `r text_spec("weekday", color="black", background="#EEEEEE")`, the intercept of the model, $\alpha$ is _not_ significantly different from zero.
...
As noted by @iriz, the `r text_spec("caret", color="black", background="#EEEEEE")`-package [@kuhn] provides a convenient way to train machine-learning algorithms using cross-validation (CV). Furthermore, the package standardizes R functions across several libraries. In the following, I am using `r text_spec("caret", color="black", background="#EEEEEE")` to tune and fit more flexible algorithms. 
...
The argument `r text_spec("getModelInfo('rf')$rf$parameters", color="black", background="#EEEEEE")` reveals that the random forest algorithm of the package `r text_spec("randomForest", color="black", background="#EEEEEE")` [@rf] only has one tuning parameter, i.e. `r text_spec("mtry", color="black", background="#EEEEEE")`. As mentioned above, for each decision tree that is fitted, random forest _randomly_ selects the predictors to be used. `r text_spec("mtry", color="black", background="#EEEEEE")` sets the _number_ of features to be picked at random for each tree. As there are 7 independent variables available, it is most sensible to consider a value for `r text_spec("mtry", color="black", background="#EEEEEE")`$< 7$. Using 10-fold cross-validation, I am tuning `r text_spec("mtry", color="black", background="#EEEEEE")` on the `r text_spec("train", color="black", background="#EEEEEE")` set. I am considering values `r text_spec("mtry", color="black", background="#EEEEEE")`$\in \{1,2, ..., 6\}$.

The random forest algorithm to be used is developed by Breiman et all, 2018 and is available in the `r text_spec("randomForest", color="black", background="#EEEEEE")`-library.
...
tuning_result$results %>%
  ...
  theme(panel.border = element_blank(),
        ...
        plot.title=element_text(size=15, family="sans"))
```
Observe that the random forest model is best performing for `r text_spec("mtry", color="black", background="#EEEEEE")`$=2$. Using this parameter value, I am fitting the random forest model on the `r text_spec("train", color="black", background="#EEEEEE")` set and summarize the RMSE performance of predictions on the `r text_spec("test", color="black", background="#EEEEEE")` set.
...
* `r text_spec("eta", color="black", background="#EEEEEE")`: Shrinkage of feature weights. Default value $0.3$, $\textrm{eta} \in [0,1]$
* `r text_spec("lambda", color="black", background="#EEEEEE")` ($\lambda$): L2 regularization term on weights, default value $1$
* `r text_spec("alpha", color="black", background="#EEEEEE")` ($\alpha$): L1 regularization term on weights, default value $0$
* `r text_spec("nrounds", color="black", background="#EEEEEE")`: 50~150

Going forward, I am accepting the default values for `r text_spec("eta", color="black", background="#EEEEEE")` and `r text_spec("nrounds", color="black", background="#EEEEEE")` and tune the $\alpha$ and $\lambda$ parameters.
...
tuning_result$results %>%
  ...
  theme(panel.border = element_blank(),
        ...
        plot.title=element_text(size=15, family="sans"))
```
As seen in the plot above, the lowest cross-validated RMSE on the `r text_spec("train", color="black", background="#EEEEEE")` set is achieved for $\lambda = \alpha = 0.019$. Analog to the previous section, I am applying the model on the out-of-sample `r text_spec("test", color="black", background="#EEEEEE")` set.
...
```
Gradient boosting significantly outperforms random forest in terms of `r text_spec("test", color="black", background="#EEEEEE")` set's RMSE. Still, it fails to achieve the performance of the linear model variants. 
...
Applying the SVM algorithm to many predictors can be achieved efficiently by using a kernel function. Using the `r text_spec("kernlab", color="black", background="#EEEEEE")` library [@svm], I am considering a linear kernel function and the more flexible radial basis kernel. 

### Linear Kernel

For the SVM algorithm with linear kernel, there is only one parameter to tune, i.e. `r text_spec("C", color="black", background="#EEEEEE")` (cost). The cost parameter determines the margin of the separating hyperplane. A low value of `r text_spec("C", color="black", background="#EEEEEE")` is associated with a large margin, a high value is associated with a small margin. Going forward, I am considering values `r text_spec("C", color="black", background="#EEEEEE")`$\in[0.5, 1.2]$.
...
#  mutate(nrounds = as.factor(nrounds)) %>%
  ...
  theme(panel.border = element_blank(),
        ...
        plot.title=element_text(size=15, family="sans"))
```
According to the 25-fold cross validation procedure on the `r text_spec("train", color="black", background="#EEEEEE")` set, a parameter value around $0.7$ seems to be the optimal choice (`r text_spec("C", color="black", background="#EEEEEE")`$=0.6842$).
...
```
So far, the SVM with linear kernel achieves the best RMSE performance on the out-of-sample `r text_spec("test", color="black", background="#EEEEEE")` set. It beats the second best weekday effect model's RMSE by `r round(100*(0.0008568/0.0008178 -1),2)`%.

### Radial Basis Function

Below, I am tuning a SVM with the more flexible radial basis kernel. For the parameter values to be tuned, `r text_spec("C", color="black", background="#EEEEEE")` and `r text_spec("sigma", color="black", background="#EEEEEE")` ($\sigma$). For the tuning grid I am considering values `r text_spec("C", color="black", background="#EEEEEE")`$\in[3, 3.6]$ and $\sigma\in[0.003, 0.007]$.
...
#  mutate(nrounds = as.factor(nrounds)) %>%
  ...
  theme(panel.border = element_blank(),
        ...
        plot.title=element_text(size=15, family="sans"))
```
The optimal parameter values according to 10-fold cross-validation on the `r text_spec("train", color="black", background="#EEEEEE")` set are `r text_spec("C", color="black", background="#EEEEEE")`$= 3.3$ and $\sigma= 0.005$.
...
The k nearest neighbor algorithm classifies a data point by looking at the outcomes of the $k$ _closest_ points to it. For regression, the mean y-variable value of the k closest points is assigned. For the tuning parameter `r text_spec("k", color="black", background="#EEEEEE")` I am considering values `r text_spec("k", color="black", background="#EEEEEE")`$\in[20,36]$.
...
According to 20-fold cross validation on the `r text_spec("train", color="black", background="#EEEEEE")` set, the optimal value for `r text_spec("k", color="black", background="#EEEEEE")` is 29.
...
When it comes to out-of-sample performance on the `r text_spec("test", color="black", background="#EEEEEE")` set, the KNN algorithm performs worst. In comparison to the support vector machine with linear kernel the RMSE performance is `r round(100*(0.0009446/0.0008178 -1),2)`% worse.

# Validation Set Performance

The table above summarizes the `r text_spec("test", color="black", background="#EEEEEE")` set's RMSE performance of all models considered. The best performing models _per algorithm_ are:
...
Going forward, these four models are fit to the _complete_ `r text_spec("training", color="black", background="#EEEEEE")` set and evaluated on the `r text_spec("validation", color="black", background="#EEEEEE")` set. For tuning on the `r text_spec("training", color="black", background="#EEEEEE")` set, for each model, I am considering a grid range of $\pm 50\%$ of the previously obtained parameter values:
...
On a first look, of all methods and models considered, the gradient boosted tree algorithm performs best on the out-of-sample `r text_spec("validation", color="black", background="#EEEEEE")` set. Note that on the first pseudo out-of-sample `r text_spec("test", color="black", background="#EEEEEE")` set the support vector machine with linear kernel performed best. I the following, I am analyzing the results in greater detail.
...
1. During 'normal' times, the support vector machine algorithm with linear kernel performs best. This is confirmed by two out-of-sample datasets: The `r text_spec("test", color="black", background="#EEEEEE")` set's and `r text_spec("validation", color="black", background="#EEEEEE")` _pre-crisis_ period's RMSE. 
2. During periods of severe market turmoil, the gradient boosted tree algorithm performs best. This is confirmed by investigating the out-of-sample `r text_spec("validation", color="black", background="#EEEEEE")` set which is heavily impacted by the covid-19 pandemic crisis.
...
1. In this analysis, only 6 different models are considered. However, the `r text_spec("caret", color="black", background="#EEEEEE")` library [@kuhn] to date provides 238 algorithms to choose from. It is questionable that the best performing model is included in the investigated models of this paper
...
Furthermore, in this application, the models are fitted _once_ on the `r text_spec("training", color="black", background="#EEEEEE")` set and then applied over the _complete period_ of the `r text_spec("validation", color="black", background="#EEEEEE")` set. A more rigorous approach would be to only predict _one day_ of the hold-out set and then re-estimating the model on a rolling basis of observations. However, this approach is computationally costly, and _each model_ considered had to be re-trained (and possibly re-tuned) `r nrow(validation)` times to obtain a RMSE measure on the `r text_spec("validation", color="black", background="#EEEEEE")` set.
...
Using _external information_, it is possible to merge daily EUR/USD quotes, derive the continuously compounded, squared return, fit a GARCH(1,1) model on the `r text_spec("training", color="black", background="#EEEEEE")` set and compare the GARCH(1,1) RMSE performance to the other algorithms on the `r text_spec("validation", color="black", background="#EEEEEE")` set.

```{r add-external-data}
# load external data, daily EUR/USD close
dt <- fread("data/EURUSD_quotes.csv", sep=",", header=TRUE, stringsAsFactors=FALSE, integer64="numeric", na.strings="")
...
Analog to the previous procedures, I am fitting the GARCH(1,1) model on the `r text_spec("training", color="black", background="#EEEEEE")` set and investigate the prediction performance on the `r text_spec("validation", color="black", background="#EEEEEE")` set.
...
Below, I am visualizing the _out-of-sample_ `r text_spec("validation", color="black", background="#EEEEEE")` predictions $\hat\sigma_{t+1}$ vs. the actual (realized) volatility $RV_{t+1}$.
