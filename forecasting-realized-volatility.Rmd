...
tuning_result$results %>%
  ...
  theme(panel.border = element_blank(),
        ...
        plot.title=element_text(size=15, family="sans"))
```
As seen in the plot above, the lowest cross-validated RMSE on the `train` set is achieved for $\lambda = \alpha = 0.019$. Analog to the previous section, I am applying the model on the out-of-sample `r text_spec("test", color="black", background="#EEEEEE")` set.
...
# rename variables
names(tbl)[2:5] <- c("Actual", "Linear (Weekday Effect)", "GBT", "SVM (Linear Kernel)", "KNN")
